...
const SCORE_LABEL = { 0: '0', 1: '1', 2: '2' };

function scoreTableHtml(items, form, groupCode, groupTitle) {
  const rows = items.map(({ code, label }) => {
    const v = form[code];
    const scoreText = (v !== null && v !== undefined) ? SCORE_LABEL[v] : '';
    return `<tr>
      <td style="width:70px;font-family:monospace">${code.toUpperCase()}</td>
      <td>${label}</td>
      <td style="width:130px;text-align:center;font-weight:bold">${scoreText}</td>
    </tr>`;
  }).join('');
  const groupRow = groupCode
    ? `<tr class="group-row"><td>${groupCode}</td><td>${groupTitle}</td><td></td></tr>`
    : '';
  return `<table class="score-table" style="width:100%;margin-bottom:8px">
    <thead><tr>
      <th style="width:70px">Kod</th>
      <th>Göstərici</th>
      <th style="width:130px;text-align:center">0 – Tam məhdud<br>1 – Məhdud<br>2 – Yaxşı</th>
    </tr></thead>
    <tbody>${groupRow}${rows}</tbody>
  </table>`;
}

function sectionH2(text) {
  return `<h3 style="font-size:12px;font-weight:bold;margin:14px 0 4px">${text}</h3>`;
}
...
const ALIF = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'];

function checkboxBlock(title, options, selectedValue) {
  const rows = options.map((o, i) => {
    const mark = selectedValue === o.value ? '✓' : '\u00A0';
    return `<div style="font-size:11px;margin-bottom:3px">${ALIF[i]}) ${o.label} (${i + 1}) &nbsp;&nbsp;[&nbsp;${mark}&nbsp;]</div>`;
  }).join('');
  return `<div style="margin-bottom:10px">
    <div style="font-weight:bold;font-size:11px;margin-bottom:4px">${title}</div>
    ${rows}
  </div>`;
}
...
function blankLine(label, value, width) {
  ...
  return `<div style="display:flex;align-items:flex-end;gap:8px;margin-bottom:6px;width:${w}">
    ...
    <span style="flex:1;border-bottom:1px solid #000;font-size:11px;padding-bottom:1px">${val(value)}</span>
  </div>`;
}
...
export async function generateUsaqPdf(usaqId) {
  ...
  function sigRow(ad, vezife) {
    return `<tr>
      ...
      <td style="padding:6px 8px;font-size:11px;width:160px;border-bottom:1px solid #000"></td>
...
<style>
  * { box-sizing: border-box; }
  body { font-family: Arial, sans-serif; margin: 0; padding: 20px 28px; color: #000; font-size: 11px; }
  h1 { font-size: 12px; text-align: center; margin: 0; font-weight: bold; }
  h2 { font-size: 12px; text-align: center; font-weight: bold; margin: 4px 0 16px; }
  .section-title { font-size: 12px; font-weight: bold; margin: 14px 0 6px; }
  table { border-collapse: collapse; }
  .info-table { width: 100%; margin-bottom: 8px; }
  .info-table td { padding: 2px 4px; font-size: 11px; vertical-align: top; }
  .label { width: 200px; }
  .blank { border-bottom: 1px solid #000; min-width: 120px; }
  ...
  .score-table th, .score-table td { border: 1px solid #000; padding: 3px 6px; font-size: 10px; vertical-align: top; }
  .score-table th { font-weight: bold; text-align: left; background: #fff; }
  .score-table .group-row td { font-weight: bold; background: #fff; }
...
<h2>${val(y.komissiya_seheri) || '_________________'} Psixoloji–tibbi–pedaqoji komissiyasının QƏRARI</h2>
...
<div style="display:flex;gap:24px;margin-bottom:4px">
  <div style="flex:1">
    ${checkboxBlock('H.8. Təhsil alma forması:', TEHSIL_FORMASI_OPTIONS, y.h8_tehsil_formasi)}
  </div>
  <div style="flex:1">
    ${checkboxBlock('H.9. Təhsil müəssisəsinin tipi:', MUESSISE_TIPI_OPTIONS, y.h9_muessise_tipi)}
  </div>
</div>

${checkboxBlock('H.10. Təhsil proqramı:', TEHSIL_PROQRAMI_OPTIONS, y.h10_tehsil_proqrami)}

<div style="font-size:11px;font-weight:bold;margin-bottom:2px">K.6. Əsas:</div>
<div style="font-size:11px;border:1px solid #000;padding:4px 8px;margin-bottom:12px;min-height:36px">${val(y.k6_esas)}</div>
...
<div style="border-top:2px solid #000;margin:20px 0 16px"></div>
...
<table style="width:100%;border-collapse:collapse;margin-top:8px">
  <tr>
    <th style="text-align:left;padding:4px 8px;font-size:11px;border-bottom:1px solid #000">A.S.</th>
    <th style="text-align:left;padding:4px 8px;font-size:11px;border-bottom:1px solid #000">Vəzifə</th>
    <th style="text-align:left;padding:4px 8px;font-size:11px;border-bottom:1px solid #000;width:180px">İmza</th>
  </tr>
  <tr><td style="padding:6px 8px;font-size:11px">${sedr}</td><td style="padding:6px 8px;font-size:11px">Sədr</td><td style="padding:6px 8px;border-bottom:1px solid #000"></td></tr>
  <tr><td style="padding:6px 8px;font-size:11px">${katib}</td><td style="padding:6px 8px;font-size:11px">Katib</td><td style="padding:6px 8px;border-bottom:1px solid #000"></td></tr>
  <tr><td style="padding:6px 8px;font-size:11px">${uzv1}</td><td style="padding:6px 8px;font-size:11px">Üzv</td><td style="padding:6px 8px;border-bottom:1px solid #000"></td></tr>
  <tr><td style="padding:6px 8px;font-size:11px">${uzv2}</td><td style="padding:6px 8px;font-size:11px">Üzv</td><td style="padding:6px 8px;border-bottom:1px solid #000"></td></tr>
  <tr><td style="padding:6px 8px;font-size:11px">${uzv3}</td><td style="padding:6px 8px;font-size:11px">Üzv</td><td style="padding:6px 8px;border-bottom:1px solid #000"></td></tr>
</table>
